...
namespace XSNotifications
{
    public class XSNotifier : IDisposable
    {
        ...
        public static MelonPreferences_Entry<int> xsoPort;
        private static XSNotifier _instance;
        public static XSNotifier Instance 
        {
            get
            { 
                ...
                {
                    validateConfig();
                    _instance = new(null, xsoPort.Value);
                }
                return _instance; 
            } 
        }
        ...
        private static void validateConfig()
        {
            if (xsoPort is null)
            {
                xsoPort = MelonPreferences.CreateEntry("ReModCore", "xsoPort", 42069, "XSO Port", "Selects what network port to send udp packets on for the XSOverlay integration. (requires restart)");
            }
        }
